...
class VoiceController extends GetxController {
    ...
    try {
      bool available = await _speech.initialize(
        ...
        onError: (errorNotification) {
          print("🎤 Ses Hatası Detayı: ${errorNotification.errorMsg} - ${errorNotification.permanent}");
          isListening.value = false;
          lastCommandStatus.value = "Hata: ${errorNotification.errorMsg}";
        },
      );

      print("🎤 STT Başlatılabilir mi?: $available");
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (result) {
            print("🎤 Duyulan: ${result.recognizedWords}");
  ...
  void _checkForWakeWord(String text) {
    ...
    final wakeWordRegex = RegExp(r'\b(artemiz|artemis|arte|yunikaya|yunika)\b', caseSensitive: false);
