...
import 'package:unicotantic/Unic/fonksiyonlar/puan_controller.dart';

class YorumaYorumMetinGir extends StatefulWidget {
  var gelenKullaniciEmail;
  final postAydi;
  final postYorumID;

  YorumaYorumMetinGir({
    required this.gelenKullaniciEmail,
    required this.postAydi,
    required this.postYorumID,
  });

  @override
  State<YorumaYorumMetinGir> createState() => _YorumaYorumMetinGirState();
}

class _YorumaYorumMetinGirState extends State<YorumaYorumMetinGir> {
  ...
  late File yuklenecekgetThumbnailDosya =
      'assets/images/icon/icon256.png' as File;
...
// Resmin boyutunu alın
  ...
  galeridenVideoYukle() async {
    ...
    var compressVideoFilePath = await VideoCompress.compressVideo(
        alinanDosyaVideo!.path,
        quality: VideoQuality.LowQuality);
    var compressVideoFilePathFile = compressVideoFilePath!.file;

    ///
    var getThumbnail =
        await VideoCompress.getFileThumbnail(alinanDosyaVideo.path);
    print('ryyjyrj yjy5 k5e5yk yksy kuyk dyk sytkdytkdt kdtktkktkk tkdt y' +
    ...
    if (yuklenecekDosyab > maksimumBoyut) {
      ...
      UploadTask yuklemeGoreviThumbnail =
          referansYolFoto.putFile(yuklenecekgetThumbnailDosya);
  ...
  metinEkle() async {
    ...
    String yorumYapanEmail = puanSa.get('yorumYapanEmail').toString();
    String kimeYorumDegistir = puanSa.get('kimeYorumDegistir').toString();
    ...
    var icerik1 = kullanicilar.doc(widget.gelenKullaniciEmail.toString());

    if (widget.gelenKullaniciEmail != controllerNet.kullanici.email) {
      await icerik1.update({'bildirim': FieldValue.increment(1)});
      await icerik1.update({'unic': FieldValue.increment(1)});
    }

    if (yorumYapanEmail.toString() != '') {
      widget.gelenKullaniciEmail = kimeYorumDegistir;
    }
    ...
    await FirebaseFirestore.instance
        ...
        .set({
      'id': '@' + id.toString(),
      'postAydi': widget.postAydi.toString(),
      'postYorumID': widget.postYorumID.toString(),
